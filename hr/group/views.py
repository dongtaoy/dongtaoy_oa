...
def group_detail(request):
    ...
    try:
        spec_group = Department.objects.get(id=request.GET.get('group_id'))
    except Exception:
        spec_group = None
...
def group_mod(request):
    with transaction.atomic():
        Department(id=request.POST.get('group_id'),
...
def group_delete(request):
    Department.objects.get(id=request.POST.get('group_id')).delete()
    groups = Department.objects.all()
    return render(request, 'hr/group/body.html', {"success": True,
                                                  "groups": groups})